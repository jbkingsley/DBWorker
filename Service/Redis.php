...
class Redis extends \DBWorker\Service
{
    protected $config = [
        'host' => '127.0.0.1',
        'port' => 6379,
    ...
    {
        if ($config) {
            (new \DBWorker\Parameter\Validator())->execute($config, [
    ...
    public function connect(): \Redis
    {
        ...
        if (!$conn) {
            throw new \DBWorker\Service\Exception('Cannot connect redis');
        }

        if ($config['password'] && !$handler->auth($config['password'])) {
            throw new \DBWorker\Service\Exception('Invalid redis password');
        }

        if ($config['database'] && !$handler->select($config['database'])) {
            throw new \DBWorker\Service\Exception('Select redis database[' . $config['database'] . '] failed');
        }
