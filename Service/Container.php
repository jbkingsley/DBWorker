...
class Container extends \DBWorker\Container
{
    ...
    protected function setService(string $id, array $options)
    {
        $this->set($id, function () use ($options) {
            ...
            $class = $options['class'];
            if (!is_subclass_of($class, '\DBWorker\Service')) {
                throw new \UnexpectedValueException('Require subclass of "\DBWorker\Service"');
            }
