...
abstract class Data implements \JsonSerializable
{
     ...
     * @var string
     */
    protected static $mapper = '\DBWorker\DataMapper\Mapper';
     ...
     * @return $this
     *
     * @throws \DBWorker\DataMapper\Exception\UndefinedPropertyException 如果属性未定义
     * @throws \DBWorker\DataMapper\Exception\UnexpectedPropertyValueException 把null赋值给一个不允许为null的属性
     * @throws \DBWorker\DataMapper\Exception\UnexpectedPropertyValueException 值没有通过设定的正则表达式检查
     * @throws \DBWorker\DataMapper\Exception\DeprecatedPropertyException      属性被标记为“废弃”
     * @throws \DBWorker\DataMapper\Exception\RefuseUpdatePropertyException    属性不允许更新修改
     */
    ...
    public function setIn(string $key, $path, $value, bool $push = false): self
    {
        ...
        \DBWorker\array_set_in($target, $path, $value, $push);
    ...
    public function unsetIn(string $key, $path): self
    {
        ...
        \DBWorker\array_unset_in($target, $path);
        $this->change($key, $target);
    ...
    public function getIn(string $key, $path)
    {
        ...
        return \DBWorker\array_get_in($target, $path);
    }
     ...
     * @return true
     *
     * @throws \DBWorker\DataMapper\Exception\UnexpectedPropertyValueException
     */
    public function validate(): bool
    {
        ...
        foreach ($keys as $key) {
            ...
            if ($type->isNull($value)) {
                ...
                if (!$attribute['allow_tags'] && is_string($value) && \DBWorker\str_has_tags($value)) {
    ...
    public static function findOrFail($id): self
    {
        ...
        throw new \DBWorker\DataMapper\Exception\DataNotFoundException();
    }
